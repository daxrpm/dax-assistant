# Graph Report - dax-assistant  (2026-07-20)

## Corpus Check
- 401 files · ~592,420 words
...
7. `Message` - 110 edges
8. `s()` - 105 edges
9. `SecretStore` - 97 edges
10. `VoicePipeline` - 95 edges

## Surprising Connections (you probably didn't know these)
- `ModelSelector()` --indirect_call--> `m()`  [INFERRED]
  web/src/pages/Chat.tsx → src/dax/web/static/assets/index-CIYnMraU.js
...
Cohesion: 0.11
Nodes (98): a(), ar(), Au(), Av(), b(), Ba(), bj(), br() (+90 more)

### Community 7 - "Application Storage Lifecycle"
Cohesion: 0.05
Nodes (51): datetime, In-memory log buffer + live fan-out for the web Logs viewer.  A single :class:`L, ChannelType, Conversation, Language, Message, MessageRole, StrEnum (+43 more)

### Community 8 - "Agent Message Processing"
Cohesion: 0.12
Nodes (17): canonical_name(), Authentication and session security.      Secrets are supplied via environment v, SecurityConfig, Requested tool does not exist in the registry., ToolNotFoundError, _connect(), FakeWebSocket, _hello() (+9 more)
...
Cohesion: 0.08
Nodes (16): Any, Tool registry — aggregates tools from all MCP servers.  Provides lookup by tool, Return all registered tool schemas., Return the tool_name → server_name mapping., Look up which server owns a tool., Return the most relevant tools for a given query.          Uses keyword matching, Aggregates tool schemas from multiple MCP servers.      Maintains a mapping of t, Register tools from an MCP server.          Each tool dict must include a 'serve (+8 more)

### Community 11 - "Collection Cursor Operations"
Cohesion: 0.04
Nodes (37): is_canonical_shell(), The result of an MCP tool execution., ToolResult, LLMProvider, Protocol, Protocol interfaces (ports) for the hexagonal architecture.  All adapters implem, Launch and connect to all configured MCP servers., Shut down all MCP server connections. (+29 more)
...
Cohesion: 0.10
Nodes (42): C, BackendResolution, BackendSettings, BackendState, BackendStateInner, BackendStrategy, decode_settings(), HealthResponse (+34 more)

### Community 16 - "Speech Synthesis Engines"
Cohesion: 0.07
Nodes (34): LogEntry, AgentEvent, boundMessages(), ChatMessage, ChatSnapshot, ChatStatus, ChatStore, chatStores (+26 more)

### Community 17 - "Shared Web UI Components"
Cohesion: 0.07
Nodes (16): NodesConfig, Device, DeviceRegistry, generate_device_secret(), _now(), Enrolled client devices and their credentials.  A single password is the right e, Populate the in-memory mirror. Call once after the database starts., True when the device exists and has not been revoked. (+8 more)

### Community 18 - "Configuration API Routes"
Cohesion: 0.03
Nodes (59): PiperVoice, Voice pipeline configuration., VoiceConfig, Text-to-speech synthesis failed., TTSError, _build_local_tts(), _build_piper(), build_tts() (+51 more)

### Community 19 - "MCP Configuration Routes"
Cohesion: 0.09
Nodes (17): Constant-time-ish check of a presented device secret., app(), bus(), client(), _enroll(), AsyncClient, FastAPI, Path (+9 more)

### Community 20 - "System API Tests"
Cohesion: 0.05
Nodes (25): Message, Web channel — delegates to WebSocket manager.  The actual WebSocket handling is, Web UI channel adapter.      Bridges between the dispatcher and the WebSocket ma, Broadcast a message to all connected WebSocket clients., WebChannel, app(), bus(), client() (+17 more)

### Community 21 - "Webhook API Tests"
Cohesion: 0.05
Nodes (38): MessageBus, Message, Async message bus for inter-component communication.  Decouples channels from th, Async message bus using asyncio queues.      Thread-safe for posting from voice, Initialize the queues. Must be called from an async context., Publish a message from a channel to the orchestrator., Wait for and return the next inbound message., Publish a response from the orchestrator to a channel. (+30 more)

### Community 22 - "System MCP Server"
Cohesion: 0.08
Nodes (24): FastMCP, main(), Run the dax-system MCP server over stdio: python -m dax.mcp_servers.system, allowed_roots(), build_server(), _command_environment(), configured_shell_allowlist(), Path (+16 more)

### Community 23 - "Selection and Syntax Utilities"
Cohesion: 0.10
Nodes (21): A request to execute an MCP tool., ToolCall, _MockLLM, _MockStorage, _MockTools, Any, Message, Tests for the orchestrator agent with LLM + tool calling. (+13 more)
...
Cohesion: 0.13
Nodes (18): MCPPreset, RegistryServer, envToText(), FormMode, headersToText(), parseEnv(), parseHeaders(), ServerFormModal() (+10 more)

### Community 26 - "LLM Router Failover"
Cohesion: 0.11
Nodes (13): LLMProviderUnavailableError, No LLM provider is available to handle the request., LLMRouter, Any, Message, LLM router — local-first fallback across decoupled providers.  Holds an ordered, Routes completion requests across an ordered list of providers., Swap the provider list in place (e.g. after a config change).          Mutates t (+5 more)
...
Cohesion: 0.13
Nodes (29): Cu(), Du(), ed(), ei(), Er(), Eu(), Fu(), Gu() (+21 more)

### Community 34 - "MCP Lifecycle Manager"
Cohesion: 0.11
Nodes (15): MCPServerConfig, MCPManager, Any, Build an unconnected client for a server config (env resolved)., Launch and connect to all enabled MCP servers., Connect to a server and register its tools live. Returns tool count.          Re, Disconnect a server (if connected) and drop its tools., Disconnect a server from the MCP lifecycle worker task. (+7 more)

### Community 35 - "Secure Configuration Serialization"
Cohesion: 0.10
Nodes (32): Mirror the live shell allowlist into encrypted configuration., _del_path(), dump_config_toml(), _env_var_for_header(), _env_var_for_mcp_env(), _extract_secrets(), _get_path(), _is_sensitive_header() (+24 more)

### Community 36 - "Realtime Chat Interface"
Cohesion: 0.12
Nodes (17): ConversationSummary, AgentEvent, boundChatMessages(), ChatMessage, ConfirmationRequest, nextId(), shouldAcceptChatFrame(), Status (+9 more)

### Community 37 - "Frontend Runtime Dependencies"
Cohesion: 0.05
Nodes (100): api, FullConfig, MemoryType, VoicePreviewOptions, AlertIcon(), CheckIcon(), PlayIcon(), PlusIcon() (+92 more)
...
Cohesion: 0.06
Nodes (43): ApiError, authHeaders(), request(), requestBlob(), requestForm(), responseError(), getBaseUrl(), AuthStatus (+35 more)
...
Cohesion: 0.13
Nodes (15): approval_from_app(), auth_from_app(), bus_from_app(), _device_for(), WebSocket, WebSocket chat endpoint for the web UI.  Handles inbound messages from browser c, Record that *websocket* owns ``session_id``.          Called when a client publi, Atomically add session ownership, respecting the per-client cap. (+7 more)

### Community 55 - "Wake Word Detection"
Cohesion: 0.05
Nodes (57): AppShell(), THEME_ORDER, CommandPalette(), fold(), PALETTE_ROUTES, PaletteAction, PaletteRoute, usePaletteShortcut() (+49 more)
...
Cohesion: 0.08
Nodes (14): is_auto_allowable(), Runtime allowlist of shell binaries the assistant may run on this PC.  This is t, Mutable command preference list constrained by read-only profiles., Return whether a full command may run without human approval., Append a binary if new. Returns True if it was actually added., Replace the whole list (de-duped, order preserved) and persist., Extract the bare binary name from a command string (``/bin/ls -l`` → ``ls``)., Return whether ``command`` has a bounded, read-only argv shape.      This is int (+6 more)

### Community 58 - "Configuration Serialization Tests"
Cohesion: 0.10
Nodes (16): AppLanguage, ENGLISH, SPANISH, AppPreferences, AppPreferenceState, fromStored(), Boolean, StateFlow (+8 more)

### Community 59 - "Domain Error Hierarchy"
Cohesion: 0.11
Nodes (18): Audio troubleshooting, Choosing / adding LLM providers, Configuration, Dax Assistant, Desktop client, Development, Development quick start, Highlights (+10 more)
...
Cohesion: 0.15
Nodes (11): LLMError, LLMTimeoutError, LLM provider communication failed., LLM request timed out., CodexProvider, Any, Message, OpenAI Codex CLI provider.  Runs ``codex exec --json`` as a subprocess to use th (+3 more)
...
Cohesion: 0.13
Nodes (32): index(), api, Field(), Panel(), PanelHeader(), Tabs(), TextArea(), TextInput() (+24 more)
...
Cohesion: 0.06
Nodes (47): usesRemoteAudio(), MCPServerStatus, ToolAuditEntry, CommandDeck(), LiveOrb(), Meter(), MetricsPane(), PIPELINE_KEY (+39 more)
...
Cohesion: 0.18
Nodes (25): CompletedProcess, Path, Release and installer contract tests., _release_fixture(), _run_installer(), _run_mocked_backend_upgrade(), _sha256(), test_installer_attestation_bypass_is_explicit_and_loud() (+17 more)
...
Cohesion: 0.27
Nodes (18): backup_database(), die(), fetch(), info(), install_backend(), install_desktop(), install_node(), install_source() (+10 more)
...
Cohesion: 0.08
Nodes (28): build_messages_for_llm(), Any, Message, Shared LLM helpers: the system prompt and the message builder.  The conversation, Build the OpenAI-format message list for an LLM call.      Converts our Message, Remove provider control markup that must never reach users or TTS., sanitize_assistant_text(), Agent (+20 more)

### Community 153 - "getFullNode"
Cohesion: 0.10
Nodes (20): ApiError, AuthStatus, ConversationDetail, ConversationMessage, DeviceKind, OllamaModel, PairCodeResponse, PairedDevice (+12 more)
...
Cohesion: 0.17
Nodes (8): Devices with a live chat socket right now., Manages active WebSocket connections and their session interests.      A single-, WebSocketManager, _attach(), No eligible client means the gate must fail safe, not fan out., Legacy clients that never send a session_id must keep working., TestApprovalDelivery, TestSessionScopedDelivery
...
Cohesion: 0.14
Nodes (10): Closed, Frame, ByteArray, String, WebSocket, VoiceSocket, VoiceSocketEvent, VoiceSocketTest (+2 more)
...
Cohesion: 0.10
Nodes (19): bus(), client(), _enroll_client(), _enroll_node(), _PresentHub, AsyncClient, FastAPI, Capability-node policy: who may read it, who may change it, what it means. (+11 more)
...
Cohesion: 0.11
Nodes (14): Allow / ask / deny policy for tool execution (fnmatch patterns).      An empty `, ToolPolicyConfig, Decision, StrEnum, Tool execution policy — allow / ask / deny per tool name.  The agent consults th, Resolves an allow/ask/deny decision for a tool name., Update rules in place so the agent picks them up without a restart., ToolPolicy (+6 more)
...
Cohesion: 0.12
Nodes (37): _acquire(), FakePipeline, _make_app(), Tests for the /ws/voice event stream.  The subscriber lifecycle matters more tha, A client must get a definite starting state, not silence., Connecting mid-conversation must not render as idle., A leaked subscriber would keep the pipeline metering forever., The failure that would otherwise mute the backend permanently. (+29 more)
...
Cohesion: 0.12
Nodes (28): ArgumentParser, add_edge_parser(), edge_main(), _enroll(), Namespace, Command-line interface for the capability-node daemon., _run(), default_credentials_path() (+20 more)
...
Cohesion: 0.33
Nodes (3): Release And Installation, Source Development Mode, Verified Linux Install
...
Cohesion: 0.13
Nodes (12): backoff_delay(), Capped full-jitter exponential reconnect delay., One persistent stdio session to the bundled dax-system MCP server., SystemExecutor, _FakeConnection, _FakeExecutor, test_backoff_is_exponential_jittered_and_capped(), test_daemon_refreshes_token_and_reconnects_with_backoff() (+4 more)
...
Cohesion: 0.10
Nodes (27): DiagnosticsUiState, DiagnosticsViewModel, Boolean, StateFlow, String, ViewModel, DestinationBar(), DestinationItem() (+19 more)
...
Cohesion: 0.17
Nodes (23): _decode_enrollment_wav(), delete_voice_profile(), _encode_wav(), enroll_voice(), get_voice_profile(), preview_voice(), _push_to_talk(), push_to_talk_press() (+15 more)
...
Cohesion: 0.14
Nodes (15): ChatSocket, Connected, Connecting, ConnectionState, Disconnected, Failed, isTransientChatFrame(), Boolean (+7 more)

### Community 232 - "test_settings_coverage.py"
Cohesion: 0.18
Nodes (7): EdgeDaemon, Any, Protocol, The key session tickets are verified against, once the backend sends it., WebSocketConnection, ExecuteRequest, result_frame()
...
Cohesion: 0.31
Nodes (17): build(), check_versions(), copy_one(), extract(), fail(), main(), metadata(), normalized_arch() (+9 more)

### Community 235 - ".to_json"
Cohesion: 0.09
Nodes (34): CapabilityHub, CapabilitySynthesis, CapabilityTTSTransportError, _Connection, _failure(), LocalTTSRequiredError, _PendingSynthesis, RuntimeError (+26 more)
...
Cohesion: 0.10
Nodes (23): hash_password(), _main(), Single-user authentication for the web UI and API.  Dax is a personal assistant:, Return an argon2id hash of ``password``., Check ``password`` against a stored argon2 hash., verify_password(), create_app(), FastAPI (+15 more)

### Community 245 - "Speaker"
Cohesion: 0.09
Nodes (15): Channel, Message, Input/output channel for user interaction.      Channels receive messages from u, Unique channel identifier (e.g., 'voice', 'whatsapp', 'web')., Initialize and begin listening for messages., Gracefully shut down the channel., Yield incoming messages from this channel., Deliver a response message through this channel. (+7 more)
...
Cohesion: 0.18
Nodes (27): AdvancedDisclosure(), BooleanChoice(), ChoiceChip(), ChoiceRow(), choices(), DiagnosticsLink(), Boolean, List (+19 more)

### Community 256 - ".send"
Cohesion: 0.20
Nodes (13): AssistOverlay(), AssistPanel(), AssistPreviewContent(), detailText(), Boolean, Float, Int, Modifier (+5 more)
...
Cohesion: 0.10
Nodes (22): BaseSettings, PydanticBaseSettingsSource, DaxConfig, Root configuration for Dax Assistant.      Settings are loaded in order of prior, MonkeyPatch, Path, Tests for TOML config serialization + secret extraction (config_io)., A field already holding an {env:…} ref is kept verbatim, not re-stored. (+14 more)

### Community 259 - "websocket_chat"
Cohesion: 0.24
Nodes (10): active_url_requires_a_validated_selected_authority(), confirmed_authority_reset_clears_credentials_and_pin_as_one_state_change(), explicit_authority_switch_clears_the_pin(), failed_credential_clear_preserves_the_authority_pin(), loading_persists_version_two_hybrid_migration(), malformed_settings_recover_to_safe_defaults(), Self, stale_resolution_cannot_replace_newer_settings() (+2 more)
...
Cohesion: 0.15
Nodes (9): RecognitionMode, ANDROID, SERVER, SettingsPreviewContent(), Boolean, StateFlow, ViewModel, SettingsUiState (+1 more)
...
Cohesion: 0.14
Nodes (15): HelloFrame, _matches_schema(), Filter a node's advertised addresses down to ones worth handing out.      The no, Accept harmless JSON-Schema metadata while enforcing argument shape., trusted_endpoints(), trusted_inventory(), _hello(), Endpoint validation: a node proposes an address, the backend decides.  An addres (+7 more)

### Community 268 - "parse"
Cohesion: 0.11
Nodes (20): ConversationApi, ConversationApiResult, Failed, Boolean, ConversationSummary, Int, List, Nothing (+12 more)

### Community 269 - "DaxError"
Cohesion: 0.21
Nodes (7): ChatMessage, ConversationChatState, ConversationDetail, ConversationMessage, ConversationSummary, ChatThreadPreviewContent(), ConversationsViewModelTest

### Community 270 - "websocket_voice"
Cohesion: 0.11
Nodes (19): fail(), info(), message(), COLORS, ICONS, ToastProvider(), Badge(), BadgeColor (+11 more)
...
Cohesion: 0.16
Nodes (10): NodePolicyConfig, NodesConfig, What one enrolled capability node is asked to do.      Keyed by device id in :cl, Capability-node fleet settings., The stored policy for *node_id*, or the default one., Whether *node_id* may terminate a client session and run the turn., Whether the authoritative backend may route this node tool., The policy rules, independent of transport. (+2 more)

### Community 289 - ".request"
Cohesion: 0.33
Nodes (4): Any, Send data to all connected WebSocket clients., Route *data* by its ``session_id``.          Every frame carrying a session ID r, Deliver a confirmation request only to the client that can answer it.          U
...
Cohesion: 0.33
Nodes (6): Authoritative Deployment Runbook, Backup And Restore, Diagnostics And Replacement, Supported Service Deployment, Topology, Upgrades And Rollback

### Community 299 - "AttemptLimiter"
Cohesion: 0.29
Nodes (4): SecurityConfig, AttemptLimiter, Small in-memory fixed-window limiter with bounded client state., test_attempt_limiter_is_bounded_global_and_expires()
...
Cohesion: 0.16
Nodes (11): ConnectFactory, NodeCredentials, Sleep, available_local_tts_engines(), LocalTTSExecutor, One lazy, resident local engine with strictly serialized synthesis., Advertise only local runtimes; model readiness is checked lazily., SynthesizeRequest (+3 more)
...
Cohesion: 0.33
Nodes (6): 1. Backend, 2. Desktop, 3. Android, 4. Capability node, Installing Dax, Verifying the whole chain
...
Cohesion: 0.23
Nodes (8): _configure_logging(), _load_server_instance_id(), Application bootstrap and lifecycle management.  Wires all components together v, Return the authority identity, creating it in encrypted storage once., Set up structlog with console rendering., Path, test_server_instance_identity_persists_in_encrypted_store(), TestInitialSetup
...
## Knowledge Gaps
- **551 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `CHAT`, `VOICE`, `PHONE` (+546 more)
